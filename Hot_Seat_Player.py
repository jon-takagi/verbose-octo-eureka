...
class Hot_Seat_Player(Player):
    ...
    def get_turn(self):
        w = self.game.world
        scr = w.scr
        mechs = self.team.world.get_all_active_mechs()
        for i in range(len(mechs)):
            scr.addstr(i + 10, 50, mechs[i].get_specs())
        curses.echo()
        scr.move(2, 50)
        ...
        scr.addstr(2, 50, " "*11)
        targeted_commands = r"[a-zA-V]\d+ (mov|atk|spt) [a-zA-V]\d+"
